...
import { Search, X, Eye, Trash2, Pencil, Package, Clock, CheckCircle, XCircle, Truck, CreditCard, Banknote, DollarSign } from 'lucide-react';
...
function isQrPaymentOrder(o: { payment_method?: string | null }): boolean {
...
export function AdminOrdersPage() {
  ...
  const handleAdminCompletePayment = async (orderId: number) => {
    ...
    <div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map((o) => {
                ...
                const showQrConfirm = shouldShowConfirmQrPayment(o);
                return (
                  <tr key={o.order_id} style={{ borderBottom: '1px solid rgba(139,92,246,0.05)' }}>
                    <td style={{ padding: '14px 16px', fontSize: '14px', color: '#a78bfa', fontWeight: 700 }}>#{o.order_id}</td>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        {showQrConfirm && (
                          <button
                            type="button"
                            onClick={() => handleAdminCompletePayment(o.order_id)}
                            disabled={completingOrderId === o.order_id}
                            title="PATCH /payments/admin/order/{orderId}/complete"
                            style={{ padding: '7px', borderRadius: '8px', border: 'none', cursor: completingOrderId === o.order_id ? 'not-allowed' : 'pointer', background: 'rgba(16,185,129,0.15)', color: '#34d399', display: 'flex', opacity: completingOrderId === o.order_id ? 0.6 : 1 }}
                          >
                        ...
                        <button onClick={() => openEdit(o)} title="Cập nhật" style={{ padding: '7px', borderRadius: '8px', border: 'none', cursor: 'pointer', background: 'rgba(59,130,246,0.15)', color: '#60a5fa', display: 'flex' }}>
      ...
      {viewOrder && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: '560px', background: 'linear-gradient(160deg, #1a1035, #0f0e17)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: '20px', padding: '28px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {shouldShowConfirmQrPayment(viewOrder) && (
              <div style={{ marginBottom: '16px' }}>
                <button
                  type="button"
                  onClick={() => handleAdminCompletePayment(viewOrder.order_id)}
                  disabled={completingOrderId === viewOrder.order_id}
                  style={{ width: '100%', padding: '12px', borderRadius: '12px', fontSize: '14px', fontWeight: 600, border: 'none', background: 'linear-gradient(135deg, #10b981, #06b6d4)', color: '#fff', cursor: completingOrderId === viewOrder.order_id ? 'not-allowed' : 'pointer', opacity: completingOrderId === viewOrder.order_id ? 0.7 : 1 }}
                >
                  {completingOrderId === viewOrder.order_id ? 'Đang xử lý...' : 'Xác nhận đã nhận tiền (hoàn tất thanh toán)'}
                </button>
              </div>
            )}

            <button onClick={() => setViewOrder(null)} style={{ width: '100%', padding: '12px', borderRadius: '12px', fontSize: '14px', fontWeight: 600, border: '1px solid rgba(139,92,246,0.2)', background: 'transparent', color: '#9ca3af', cursor: 'pointer' }}>Đóng</button>
          </div>
        </div>
      )}
      ...
      {editOrder && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: '480px', background: 'linear-gradient(160deg, #1a1035, #0f0e17)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: '20px', padding: '28px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <div>
                ...
                <select value={editStatus} onChange={(e) => setEditStatus(e.target.value)} style={{ ...inputStyle, cursor: 'pointer' }}>
                  {ORDER_STATUSES.filter(s => s.value !== 'all').map(s => (
                    <option key={s.value} value={s.value} style={{ background: '#0f0a24', color: '#fff' }}>{s.label} ({s.value})</option>
                  ))}
                </select>
              </div>
              <div>
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '24px' }}>
              <button onClick={() => setEditOrder(null)} style={{ flex: 1, padding: '12px', borderRadius: '12px', fontSize: '14px', fontWeight: 600, border: '1px solid rgba(139,92,246,0.2)', background: 'transparent', color: '#9ca3af', cursor: 'pointer' }}>Hủy</button>
              <button onClick={handleUpdate} disabled={saving} style={{ flex: 1, padding: '12px', borderRadius: '12px', fontSize: '14px', fontWeight: 600, border: 'none', background: 'linear-gradient(135deg, #7c3aed, #3b82f6)', color: '#fff', cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>
