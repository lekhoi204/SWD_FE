...
export function ManagerProductsPage() {
  ...
  const [form, setForm] = useState({ name: '', category_id: '', price: '', stock: '', description: '', brand: '', status: 'Available' });
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [prods, cats] = await Promise.all([getProductsApi(), getCategoriesApi()]);
      setProducts(prods);
      setCategories(cats);
  ...
  const openCreate = () => {
    setForm({ name: '', category_id: categories[0]?.category_id?.toString() || '', price: '', stock: '', description: '', brand: '', status: 'Available' });
    setImageFile(null);
  ...
  const closeModal = () => { setModal(null); setEditProduct(null); setImageFile(null); setImagePreview(null); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modal === 'create') {
        ...
        await apiClient(`/products/${editProduct.id}`, { method: 'PUT', body: formData });
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: '1 1 280px' }}>
          ...
          <input placeholder="Tìm kiếm sản phẩm..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ ...inputStyle, paddingLeft: '40px' }} />
        </div>
            ...
            <tbody>
              {filtered.map((p, i) => (
                <tr key={p.id} style={{ borderBottom: '1px solid rgba(245,158,11,0.05)' }}>
                  ...
                  <td style={{ padding: '14px 20px', fontSize: '14px', color: '#fff', fontWeight: 500, maxWidth: '300px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      {p.image && !p.image.includes('placeholder') ? (
                        <img src={p.image} alt="" style={{ width: 40, height: 40, borderRadius: '8px', objectFit: 'cover' }} />
                      ) : (
                        <div style={{ width: 40, height: 40, borderRadius: '8px', background: 'rgba(245,158,11,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <ImageIcon style={{ width: 18, height: 18, color: '#6b7280' }} />
                        </div>
                      )}
              ...
              {filtered.length === 0 && <tr><td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#6b7280', fontSize: '14px' }}>Không tìm thấy sản phẩm nào</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: '540px', background: 'linear-gradient(160deg, #1a1a0e, #0f0e17)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '20px', padding: '28px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h2 style={{ fontSize: '20px', fontWeight: 700, color: '#fff', margin: 0 }}>{modal === 'create' ? 'Thêm sản phẩm' : 'Chỉnh sửa sản phẩm'}</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  ...
                  <select value={form.category_id} onChange={(e) => setForm({ ...form, category_id: e.target.value })} style={{ ...inputStyle, cursor: 'pointer' }}>
                    <option value="" style={{ background: '#0f0a24', color: '#fff' }}>-- Chọn --</option>
                    {categories.map(c => <option key={c.category_id} value={c.category_id} style={{ background: '#0f0a24', color: '#fff' }}>{c.name}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#9ca3af', marginBottom: '6px' }}>Thương hiệu</label>
                  <input value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} style={inputStyle} placeholder="VD: MSI, ASUS..." />
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  ...
                  <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} style={{ ...inputStyle, cursor: 'pointer' }}>
                    <option value="Available" style={{ background: '#0f0a24', color: '#fff' }}>Available</option>
                    <option value="Unavailable" style={{ background: '#0f0a24', color: '#fff' }}>Unavailable</option>
                  </select>
                </div>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#9ca3af', marginBottom: '6px' }}>Mô tả</label>
                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} style={{ ...inputStyle, minHeight: '80px', resize: 'vertical' }} placeholder="Mô tả sản phẩm..." />
              </div>

              {/* Image Upload */}
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#9ca3af', marginBottom: '6px' }}>Hình ảnh sản phẩm</label>
                <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileChange} style={{ display: 'none' }} />
                <div
                  onClick={() => fileInputRef.current?.click()}
                  style={{
                    border: '2px dashed rgba(245,158,11,0.3)', borderRadius: '12px',
                ...
                >
                  {imagePreview ? (
                    ...
                    <>
                      <Upload style={{ width: 28, height: 28, color: '#f59e0b', margin: '0 auto 8px' }} />
                      <p style={{ fontSize: '14px', color: '#d1d5db', margin: '0 0 4px' }}>Nhấn để chọn ảnh từ máy</p>
                      <p style={{ fontSize: '12px', color: '#6b7280', margin: 0 }}>PNG, JPG, WEBP</p>
                    </>
                  )}
                </div>
              </div>
